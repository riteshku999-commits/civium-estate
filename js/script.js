...
// ╔══════════════════════════════════════════════════════════════╗
// ║  FOOTER — dynamic year + contact snapshot from contact.json  ║
// ╚══════════════════════════════════════════════════════════════╝
...
// Footer contact + RERA populated from contact.json (already fetched in bootstrap)
function populateFooter(contactData) {
  ...
  if (list && c) {
    list.innerHTML = `
      ...
      <li>📍 ${c.address.line2}, ${c.address.line3}</li>`;
  }

  // Social icons
  if (c?.social) {
    const icons = document.querySelectorAll(".footer-social-icon");
    const map = [c.social.facebook, c.social.instagram, c.social.linkedin, c.social.whatsapp];
    icons.forEach((a, i) => { if (map[i]) a.href = map[i]; });
  }
}


// ╔══════════════════════════════════════════════════════════════╗
// ║  LEGAL MODAL — fetch data/legal.json, render on demand       ║
// ╚══════════════════════════════════════════════════════════════╝

let legalData = null; // cached after first fetch

function openLegalModal(docKey) {
  const modal    = document.getElementById("legalModal");
  const backdrop = document.getElementById("legalModalBackdrop");
  const title    = document.getElementById("legalModalTitle");
  const eyebrow  = document.getElementById("legalModalEyebrow");
  const meta     = document.getElementById("legalModalMeta");
  const body     = document.getElementById("legalModalBody");
  if (!modal) return;

  // Show modal immediately with loading state
  modal.classList.add("legal-modal--open");
  backdrop.classList.add("legal-modal-backdrop--open");
  document.body.classList.add("legal-modal-active");
  body.innerHTML = `<p class="legal-loading">Loading…</p>`;
  setTimeout(() => document.getElementById("legalModalClose")?.focus(), 50);

  function render(data) {
    const doc = data[docKey];
    if (!doc) { body.innerHTML = `<p style="color:#888;text-align:center;">Document not found.</p>`; return; }

    const labels = { termsOfService: "Terms of Service", privacyPolicy: "Privacy Policy" };
    title.textContent   = labels[docKey] || docKey;
    eyebrow.textContent = data.company?.name || "Civium Estate";
    meta.textContent    = `Last updated: ${doc.lastUpdated}  ·  Effective: ${doc.effectiveDate}`;

    body.innerHTML = `
      <p class="legal-intro">${doc.intro}</p>
      ${doc.sections.map(s => `
        <div class="legal-section">
          <h3 class="legal-section-title">${s.title}</h3>
          <p class="legal-section-body">${s.content}</p>
        </div>`).join("")}
      <div class="legal-footer-note">
        <p>${data.company?.name} · ${data.company?.address}</p>
        <p>${data.company?.reraNumber ? "RERA: " + data.company.reraNumber : ""}</p>
      </div>`;
  ...
  if (legalData) {
    ...
    fetch("data/legal.json")
      ...
      .then(data => { legalData = data; render(data); })
      .catch(() => {
        body.innerHTML = `<p style="color:#ff6666;text-align:center;padding:40px;">
          Unable to load document. Please try again later.</p>`;
      });
  }
}
...
function closeLegalModal() {
  ...
  backdrop.classList.remove("legal-modal-backdrop--open");
  document.body.classList.remove("legal-modal-active");
}

document.addEventListener("DOMContentLoaded", () => {
  // Trigger buttons in footer
  document.querySelectorAll(".footer-legal-btn[data-doc]").forEach(btn => {
    btn.addEventListener("click", () => openLegalModal(btn.dataset.doc));
  });
  ...
  // Backdrop click
  document.getElementById("legalModalBackdrop")?.addEventListener("click", closeLegalModal);

  // Escape key
  document.addEventListener("keydown", e => {
    if (e.key === "Escape" && document.body.classList.contains("legal-modal-active"))
      closeLegalModal();
  });
});
